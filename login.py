from ttkbootstrap import *
from ttkbootstrap.constants import *
from tkinter import messagebox
import mysql.connector
import index


def login_button():
    username = "123"
    password = "123"

    if username_entry.get() == username and password_entry.get() == password:
        root.destroy()
        index.main_page() 
    else:
        messagebox.showerror(title="Login Failure", message="Login Failure")

class Login_Page:
    def __init__(self, master):
        self.top = master
        self.top.title("Login Page")

        # Frame in top
        self.input_frame = Frame(self.top)
        self.input_frame.pack()

        # Element in input frame
        global username_entry
        global password_entry

        self.login_label = Label(self.input_frame, text="Robocore CRM System", font=("Times", 12)) 
        self.username_label = Label(self.input_frame, text="username: ")
        username_entry = Entry(self.input_frame)
        self.password_label = Label(self.input_frame, text="password: ")
        password_entry = Entry(self.input_frame, show="*")
        self.login_button = Button(self.input_frame, text="Login", command=login_button)

        self.login_label.grid(row=0, column=0, columnspan=2,pady=30) 
        self.username_label.grid(row=1, column=0, padx=20, pady=10)
        username_entry.grid(row=1, column=1, padx=20, pady=10)
        self.password_label.grid(row=2, column=0, padx=20, pady=10)
        password_entry.grid(row=2, column=1, padx=20, pady=10)
        self.login_button.grid(row=3, column=0, columnspan=2, pady=30)

def log_page():
    global root
    root = Window(themename="superhero")
    root.title("Robocore CRM System Login")
    root.iconbitmap()
    root.geometry("400x300+600+300")
    global robocore_log
    robocore_log = Login_Page(root)
    root.mainloop()
    return robocore_log    

if __name__ == "__main__":
    log_page()








